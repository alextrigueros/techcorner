...
if (isset($_POST['btn_registre'])) {
    ...
    elseif (existeixEmail($conn, $email) == true) {
        $error = "Aquest correu electrònic ja està registrat.";
    } else {
        //Si no existeix, el registrem
        //Xifrem la contrasenya abans de guardar-la a la base de dades
        $password_xifrada = password_hash($password, PASSWORD_DEFAULT);
        $registre_ok = registrarUsuari($conn, $nom, $cognoms, $email, $password_xifrada);
